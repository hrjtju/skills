...
when_to_use: >-
  ...
  "section rewrite plan", "claim-evidence map", "换投", "create a resume/beamer/poster",
  "draw a TikZ diagram", "convert this PDF to LaTeX", "polish this paper", "compile error",
  "format for NeurIPS", or requests about a .tex manuscript or document.
triggers:
  - "fix my LaTeX"
  - "latex compile"
  - "proofread .* paper"
...
allowed-tools: Read, Glob, Grep, Bash(uv *), Bash(*)
---
...
| `polish`          | Academic/Chinglish prose polish (grammar → style → register) without changing meaning                           | `uv run python -B $SKILL_DIR/scripts/analyze_grammar.py main.tex --section intro`             | `references/writing/academic-phrasebank.md`, `chinglish-patterns.md`, `section-anatomy.md`, `style-guardrails.md`                       |
| `rescue`          | Compile-error diagnosis & fix (error catalog, package conflicts, stubborn chains)                              | `uv run python -B $SKILL_DIR/scripts/compile.py main.tex` (then read the log)                | `references/latex/error-catalog.md`, `package-conflicts.md`, `debug-workflow.md`                                                       |
| `pdf2tex`         | Reconstruct LaTeX source from a compiled PDF (text/math/tables/figures)                                        | (LLM + pymupdf/fitz workflow — see `references/pdf2tex/`)                                    | `references/pdf2tex/*.md`                                                                                                              |
| `document`        | Create/produce a document from scratch (resume, report, slides, poster, form, mail-merge) or convert formats    | `bash $SKILL_DIR/scripts/compile_latex.sh main.tex`                                          | `references/tooling/*.md`                                                                                                              |
| `tikz`            | Figures/diagrams in TikZ/PGF (incl. 3D plots, commutative diagrams), mermaid/graphviz/plantuml                 | (LLM-driven workflow)                                                                        | `references/latex/tikz.md`, `references/tooling/*.md`                                                                                   |
| `cleveref`        | Cross-referencing: customise `\cref`/`\Cref` format, `\crefname`, `\crefformat`, ranges, multi-refs, hyperlinks | (LLM-driven workflow)                                                                     | `references/latex/cleveref.md`                                                                                                         |

## Routing Rules

- Infer the module from the request; ask only when two or more modules are equally plausible. Run 2-3 compatible checks sequentially in this order: `compile` -> `rescue` -> `bibliography` -> `format` -> `figures` / `tikz` / `tables` / `caption` / `pseudocode` -> `grammar` / `sentences` / `deai` / `polish` -> `logic` / `literature` / `experiment` / `abstract` -> `section-writing` -> `title` / `expression` / `translation` / `adapt` -> `pdf2tex` / `document` / `cleveref`. Polish coarse-to-fine (logic -> sentences -> lexical); never reverse.
...
- `references/latex/`: LaTeX internals — `cleveref.md` (cross-referencing), `tikz.md` (TikZ/PGF, 3D, commutative), `compilation.md`, `error-catalog.md`, `package-conflicts.md`, `debug-workflow.md`.
- `references/writing/`: `style-guide.md` tone/style defaults; `academic-phrasebank.md`, `chinglish-patterns.md`, `section-anatomy.md`, `style-guardrails.md` (from latex-polish); `section-writing/` per-section guides.
- `references/venues/catalog.md` + `venue-guide.md`: venue index/templates — prefer `templates/<venue>.md` (`ieee`, `acm`, `neurips`, `icml`, `springer-lncs`) when a venue is named.
- `references/formatting/formatting-rules.md`: cross-venue formatting rules.
- `references/pdf2tex/`: PDF→LaTeX reconstruction guides.
- `references/tooling/`: document production & graphics tooling (packages, beamer, mermaid, graphviz/plantuml, conversion, charts).
...
- `references/evidence/` + `references/deai/`: claim-evidence contract, over-claim guard, de-AI tone tables.

## Unified Scope

This skill merges `/latex-paper-en`, `/latex-fmt`, `/latex-polish`, `/latex-rescue`, `/latex-document-skill`, and `/pdf2tex`.
Every sub-workflow is reachable via a module; the module router is the single entry point. Historical aliases
(`/latex-fmt`, `/latex-polish`, `/latex-rescue`, `/pdf2tex`) still route here. New knowledge added from package
documentation: `references/latex/cleveref.md`, `references/latex/tikz.md`.
