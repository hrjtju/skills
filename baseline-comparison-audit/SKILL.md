...
description: "Audit whether a paper's baseline comparisons are COMPLETE, FAIR, and SIGNIFICANT: a required recent SOTA baseline is missing while 'best/SOTA' is claimed (HP-MISSING-BASELINE); a baseline is undertuned or run at a mismatched config, or the equal-budget ablation is absent (HP-WEAK-BASELINE); 'outperforms' is asserted over overlapping error bars or with no variance/seeds (HP-SIG-OVERLAP); and a cross-row 'improves by X%' is arithmetically wrong (HP-DELTA-ERROR, cross-row form only). The EXECUTOR assembles a per-domain baseline profile + live leaderboard/recency search; a fresh cross-model reviewer PROPOSES span-anchored findings; tools/adjudicate_findings.py DECIDES. Works at L0 (stated comparisons), deepens at L2 (configs/result files); unresolvable questions become needs_external_check. Emits baseline-comparison-audit.findings.json; computes NO verdict. Detect-only. Triggers: \"baseline audit\", \"missing baselines\", \"is the comparison fair\", \"weak baseline\", \"baseline 误报\", \"SOTA earned?\"."
