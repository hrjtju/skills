...
description: "Transparent, itemized impressions of AI-generated WRITING STYLE — the repo's ONLY non-integrity track. Two passes: a deterministic defensive-hedge density screen (tools/check_ai_style.py, AIS-DEFENSIVE-HEDGE) plus a fresh cross-model GROSS-cases-only semantic pass over the 13 AIS-* style tells (narrative arc, LLM phrase tics, jargon-stuffing, invented codenames, restatement loops, single-style figures). Every finding is named, LOCATED, span-anchored to the evidence ledger (claims.json), carries not_integrity_finding:true + false_positive_risk:high + an fp_case, and gets ZERO verdict weight: the adjudicator forces it to info, excludes it from overall_verdict, and renders it in a SEPARATE report section. NOT an AI-text classifier — no scores, no \"this is AI-written\", no authorship probability. Emits ai-style-impressions.findings.json; computes NO verdict; if a tell is substantive it routes to the integrity auditor. Triggers: \"AI style\", \"vibe check\", \"writing fingerprint\", \"AI 文风\", \"vibe paper\"."
...
```bash
ROOT="${AA_ROOT:-$HOME/.agents/Anti-Autoresearch}"
