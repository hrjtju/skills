...
description: "End-to-end substantive-integrity forensic sweep of a research paper (especially autoresearch / AI-Scientist-style output). Orchestrates the whole pipeline: ingest (arxiv-id | pdf | dir → working dir + pdftotext for L0) → /evidence-ledger (artifact manifest + observability L0/L1/L2 + claims.json) → fan out the integrity auditor skills (consistency, citation, baseline, experiment, presentation, proof-derivation, eval-design — each emits span-anchored findings) + the zero-weight AIS writing-style track → advisory memos (/adversarial-case-builder, /novelty-duplication-advisory) → deterministic tools/adjudicate_findings.py (--ledger REQUIRED) → reviewer-ready Integrity Forensics Report. Cross-model (fresh codex per dimension), reviewer≠adjudicator: the model proposes findings, the deterministic adjudicator decides the verdict. Observability-aware, detect-only, never an opaque AI-text classifier. Triggers: \"anti-autoresearch\", \"integrity audit this paper\", \"forensic review\", \"audit a submission\", \"审一篇投稿的诚信\"."
...
REAL TOOLS (resolve ROOT="${AA_ROOT:-$HOME/.agents/Anti-Autoresearch}"):
...
```bash
ROOT="${AA_ROOT:-$HOME/.agents/Anti-Autoresearch}"; PAPER_DIR="<from Step 0>"
