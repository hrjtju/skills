...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Async conversation-mask SFT for Claude Opus 4.7 reasoning data.")
    parser.add_argument("--base-model", default="Qwen/Qwen3.5-4B")
    parser.add_argument("--dataset-id", default="angrygiraffe/claude-opus-4.6-4.7-reasoning-8.7k")
    parser.add_argument(
        "--dataset-path",
        type=Path,
        default=Path("datasets/claude-opus-4.6-4.7-reasoning-8.7k"),
    )
...
# SwanLab 实验名和权重名跟随当前基座模型，避免换模型后仍沿用旧名称。
SWANLAB_PROJECT = ARGS.swanlab_project
SWANLAB_MODE = ARGS.swanlab_mode
SWANLAB_EXPERIMENT_NAME = f"claude-opus47-conversation-mask-{model_slug(BASE_MODEL)}-async"
...
async def evaluate_client(client, tokenizer, prompts: list[str], title: str) -> None:
    ...
    for prompt in prompts:
        ...
        # 当前 PyTrio SDK 的 sample_async 在 await 后会直接返回 SampleResponse，不要再二次 await。
...
async def main() -> None:
    ...
    progress_bar = tqdm(total=total_steps, desc="异步 SFT 训练", unit="batch")
    for epoch in range(NUM_EPOCHS):
        log_tasks = []
        submit_bar = tqdm(
            total=steps_per_epoch,
            desc=f"第 {epoch + 1}/{NUM_EPOCHS} 轮提交",
            unit="batch",
            leave=False,
        )
        ...
        for start in range(0, len(processed_examples), BATCH_SIZE):
            ...
            # 后台等待远程任务完成并记录 loss，主循环可以继续提交后续 batch。
            async def log_train_result(
                fwdbwd_future,
                optim_future,
                batch,
                epoch,
        ...
        submit_bar.close()
        # 每个 epoch 结束前等待该轮所有 loss 记录任务完成，避免日志和进度条漏记。
