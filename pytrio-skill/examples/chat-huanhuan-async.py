...
async def main() -> None:
    ...
    progress_bar = tqdm(total=total_steps, desc="异步训练", unit="batch")
    ...
    # 训练前后都用同一组 prompt 测试，便于观察 LoRA 微调带来的变化。
    await evaluate_client(base_sampling_client, tokenizer, test_prompts, title="基础模型回答")
    await evaluate_client(tuned_sampling_client, tokenizer, test_prompts, title="微调后模型回答")
