...
## Troubleshooting (log of real incidents)

1. **Sandboxed shells (pi/Claude Code sandbox)**: `kimi-webbridge start` may
   fail with `Error: rotate log: rename .../daemon.log ...: operation not
   permitted` because the sandbox denies writes to `~/.kimi-webbridge/logs/`.
   Fix: allow the sandbox to write `~/.kimi-webbridge/` (or disable the
   sandbox).  Verified: with the sandbox enabled the daemon cannot start at
   all; with it disabled (or the path allow-listed) startup works.

2. **Daemon running but `navigate` returns `{"error":{"code":"tool_error",
   "message":"no extension connected"}}`**: the daemon is up but the browser
   extension is not connected.  The user must open/enable the Kimi WebBridge
   extension in the browser (it connects on page load).  Once connected,
   `list_tabs` / `navigate` / `snapshot` all work — simply retry.

3. **Network-proxied environments**: if direct fetch of doc sites fails, the
   browser route (this skill) is preferred; otherwise fall back to the local
   proxy (`http://127.0.0.1:7890`) with curl for static pages.
