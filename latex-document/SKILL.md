...
description: >
  ...
  professional PDF with PNG previews. Covers resumes, reports, letters,
  invoices, academic papers/theses/CVs, presentations (Beamer), posters, exams,
  books, cheat sheets, fillable PDF forms, conditional content, mail merge
  (Jinja2), version diffing (latexdiff), charts (pgfplots/matplotlib), tables
  (booktabs), images (TikZ), Mermaid diagrams, watermarks, BibTeX/biblatex
  citations, multi-language/CJK (XeLaTeX), algorithms, tcolorbox, siunitx,
  Pandoc conversion (Markdown/DOCX/HTML), and PDF-to-LaTeX (OCR of printed or
  handwritten docs). Compiles via pdflatex/xelatex/lualatex (latexmk, PDF/A).
  Use when asked to create, edit, compile, debug, or convert LaTeX documents or
  PDFs: resumes, reports, theses, slides, posters, exams, books, letters,
  forms, charts, diagrams, diffs, or PDF merge/split/extract/OCR/lint.
---
